...
# ---- 每晚睡眠评分（启发式，可调）----
# 思路：把各类声音按"每小时次数"归一化，加权成一个干扰分，再用 100 减去它得到
# 0~100 的睡眠分（越高越好）。这是经验启发式、非医学诊断，权重可按需调整。
SCORE_WEIGHTS = {
    "snore_per_hour": 0.8,    # 每小时打鼾次数的扣分权重
    "grind_per_hour": 1.0,    # 每小时磨牙次数
    "talk_per_hour": 0.5,     # 每小时梦话次数
    "disturb_pct": 1.0,       # 发声总时长占比（%）
}

# 睡眠分 -> 文字等级（下界，含）。从高到低匹配。
SCORE_LABELS = [
    (85, "优"),
    (70, "良"),
    (55, "一般"),
...
# 趋势判定阈值：连续多晚的睡眠分变化超过该值才算"变好/变差"，否则视为"基本持平"。
